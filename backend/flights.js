...
router.get('/search', async (req, res) => {
  ...
  try {
    ...
    // Filter out flights with null origin_airport or destination_airport
    const filtered = (data || []).filter(f => f.origin_airport && f.destination_airport);
    res.json(filtered);
  } catch (err) {
...
// POST /api/flights/statuses - Get status for multiple flight IDs
router.post('/statuses', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch flight statuses' });
  }
});

// POST /api/flights/batch - Get full flight data for multiple IDs
...
router.patch('/:id', async (req, res) => {
  ...
  if (!id || !status) {
    return res.status(400).json({ error: 'Missing flight ID or status' });
  }
  try {
    ...
    const { data, error } = await supabase
      ...
      .single();
    if (error || !data) {
      return res.status(404).json({ error: 'Flight not found or update failed' });
    }
